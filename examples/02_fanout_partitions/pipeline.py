"""Fan-out partition processing: one task per NYC borough, generated by a
plain Python loop -- the pattern ROADMAP.md sec 4 recommends for large
fan-out instead of a dynamic-mapping primitive baked into the core.

    extract-ish peek -> {process_<borough> for each borough} -> combine
...
connection -- the documented-safe way for independent tasks to use DuckDB
concurrently (ROADMAP.md sec 7) -- so the partitions can run in parallel
without racing on a single connection object.

Run it:

    uv run duckpipe run examples/02_fanout_partitions/pipeline.py
"""
